...
func TestHTTPRequiresConsentThenEscapesAndDelivers(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Query().Get("module") != "acme/lib & tools" || r.URL.Query().Get("literal") != "{word}" {
		...
		if payload["message"] != message || payload["module"] != "acme/lib & tools" || payload["literal"] != "{word}" {
	...
	request := contact.Request{Intent: "change", Module: "acme/lib & tools", Origin: "https://git.example/acme/lib", Message: message, Contact: manifest.Contact{
		Kind: "http", URL: server.URL + "/issues?module={module}&intent={intent}&literal={{word}}", ContentType: "application/json",
		Headers: map[string]string{"X-Static": "declared"}, Body: `{"module":"{module}","message":"{message}","literal":"{{word}}"}`,
	}}
...
func TestExecUsesArgvWithoutShellAndMCPRefuses(t *testing.T) {
	...
	request := contact.Request{Intent: "change", Module: "acme-lib; touch PWNED", Message: "$(touch PWNED)", Contact: manifest.Contact{
		Kind: "exec", Command: []string{"acme-tracker", "create"}, Args: []string{"--module", "{module}", "--literal", "{{word}}"}, Stdin: "{message}",
	}}
	record, err := driver.Deliver(context.Background(), request)
	if err != nil {
		t.Fatal(err)
	}
	if executable != "/consumer/bin/acme-tracker" || strings.Join(args, "|") != "create|--module|acme-lib; touch PWNED|--literal|{word}" || stdin != "$(touch PWNED)" || record.Driver != "exec:acme-tracker" {
