...
func TestReleaseConfigurationPreservesBinaryChannelsWithoutRemovedSubsystems(t *testing.T) {
	...
	for _, want := range []string{"contents: write", "packages: write", "id-token: write", "attestations: write", "npm publish", "pypi", "tools/release-channels.py", "tools/release-policy.py", "tools/pypi-recovery.py", "missing-wheelhouse", "cosign sign --yes", "preserve_immutable", "promote_stable", "publication_tag", "--tag \"$publication_tag\"", "docker manifest inspect"} {
