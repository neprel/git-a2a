...
func TestPolyglotConsumerFullDependencyLifecycle(t *testing.T) {
	...
	mustWrite(t, filepath.Join(consumer, "composer.json"), []byte("{\"name\":\"acme/consumer\",\"require\":{\"php\":\"^8.3\"}}\n"))
	...
	mustWrite(t, filepath.Join(consumer, "flake.nix"), []byte("{\n  description = \"consumer\";\n  outputs = { self }: {};\n}\n"))
	var out, errOut bytes.Buffer
	app := cli.New(&out, &errOut)
	app.Root = consumer
	...
	composerJSON, _ := os.ReadFile(filepath.Join(consumer, "composer.json"))
	if strings.Contains(string(composerJSON), "acme/lib") {
		t.Fatalf("Composer wiring remains:\n%s", composerJSON)
	}
	mixExs, _ := os.ReadFile(filepath.Join(consumer, "mix.exs"))
	if strings.Contains(string(mixExs), "acme_lib") {
		t.Fatalf("Hex wiring remains:\n%s", mixExs)
	}
	cabalProject, _ := os.ReadFile(filepath.Join(consumer, "cabal.project"))
	if strings.Contains(string(cabalProject), "git-a2a:begin acme-lib") {
		t.Fatalf("Hackage wiring remains:\n%s", cabalProject)
	}
	zigZON, _ := os.ReadFile(filepath.Join(consumer, "build.zig.zon"))
	...
	flakeNix, _ := os.ReadFile(filepath.Join(consumer, "flake.nix"))
	if strings.Contains(string(flakeNix), "git-a2a:begin acme-lib") {
		t.Fatalf("Nix wiring remains:\n%s", flakeNix)
	}
