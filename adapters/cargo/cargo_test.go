...
func TestWireGoldenIdempotentUnwire(t *testing.T) {
	root := t.TempDir()
	fixture := filepath.Join("..", "..", "testdata", "consumer-cargo")
	original := mustRead(t, filepath.Join(fixture, "Cargo.toml"))
	...
	if got, want := mustRead(t, filepath.Join(root, "Cargo.toml")), mustRead(t, filepath.Join(fixture, "Cargo.golden.toml")); string(got) != string(want) {
