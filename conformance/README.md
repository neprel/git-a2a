...
Protocol version 3 retains version 2's sequential case format and replaces the former selectable
`list NAME` contract with all-dependency `list` plus local owner lookup through `whose NAME`.

## Coverage

The corpus executes all six domain commands end to end against a local bare Git repository,
